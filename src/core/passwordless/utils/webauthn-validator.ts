...
export function validateWebAuthnOptions(options: Partial<WebAuthnOptions>): void {
  ...
  if (options.userVerification && !validUserVerification.includes(options.userVerification)) {
    throw new BadRequestError(`Invalid userVerification value. Must be one of: ${validUserVerification.join(', ')}`);
  }

  // Validate authenticatorAttachment
  if (options.authenticatorAttachment && !validAuthenticatorAttachment.includes(options.authenticatorAttachment)) {
    throw new BadRequestError(`Invalid authenticatorAttachment value. Must be one of: ${validAuthenticatorAttachment.join(', ')}`);
  }

  // Validate attestation
  if (options.attestation && !validAttestation.includes(options.attestation)) {
    throw new BadRequestError(`Invalid attestation value. Must be one of: ${validAttestation.join(', ')}`);
  }
