...
export class PasswordlessService {
  ...
  async startAuthentication(
    ...
    try {
      ...
      switch (method) {
        case "webauthn":
          challenge = await this.webAuthnService.generateAuthenticationChallenge(user.id, options) as Challenge
          break
        case "magic-link":
          challenge = await this.magicLinkService.sendMagicLink(user.id, identifier, options) as Challenge
          break
        default:
  ...
  async startRegistration(
    ...
    try {
      ...
      switch (method) {
        case "webauthn":
          challenge = await this.webAuthnService.generateRegistrationChallenge(userId, options) as Challenge
          break
        case "magic-link":
          // Magic link doesn't require registration, just send a magic link
          challenge = await this.magicLinkService.sendMagicLink(userId, identifier, options) as Challenge
          break
        default:
          throw new BadRequestError(`Unsupported passwordless method: ${method}`)
      }
      ...
      logger.error("Error starting passwordless registration", { error, userId, method, identifier })
      throw error
    }
  }
