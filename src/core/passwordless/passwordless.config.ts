...
const rawConfig = {
  webauthn: {
    ...
    authenticatorAttachment: process.env['PASSWORDLESS_WEBAUTHN_AUTHENTICATOR_ATTACHMENT'] as
      ...
      | 'cross-platform'
      | undefined,
    userVerification: process.env['PASSWORDLESS_WEBAUTHN_USER_VERIFICATION'] as
      | 'required'
      | 'preferred'
      | 'discouraged',
  ...
  magicLink: {
    ...
    deliveryMethod: process.env['PASSWORDLESS_MAGIC_LINK_DELIVERY_METHOD'] as
      | 'email'
      | 'sms'
      | 'both',
  ...
  smsOtp: {
    ...
    providers: process.env['PASSWORDLESS_SMS_OTP_PROVIDERS']?.split(',') as (
      | 'twilio'
      | 'aws-sns'
      | 'custom'
    )[],
  ...
  biometric: {
    ...
    userVerification: process.env['PASSWORDLESS_BIOMETRIC_USER_VERIFICATION'] as
      | 'required'
      | 'preferred'
      | 'discouraged',
    authenticatorAttachment: process.env['PASSWORDLESS_BIOMETRIC_AUTHENTICATOR_ATTACHMENT'] as
      | 'platform'
      | 'cross-platform',
    attestation: process.env['PASSWORDLESS_BIOMETRIC_ATTESTATION'] as
      | 'none'
      | 'indirect'
      | 'direct',
  },
  session: {
    ...
    cookieSameSite: process.env['PASSWORDLESS_SESSION_COOKIE_SAME_SITE'] as
      | 'strict'
      | 'lax'
      | 'none',
