...
export class WebAuthnService {
  ...
  async generateRegistrationChallenge(
    ...
    try {
      ...
      const challenge = crypto
        .randomBytes(passwordlessConfig.webauthn.challengeSize)
        .toString('base64url');
      ...
      const existingCredentials =
        await this.credentialRepository.findWebAuthnCredentialsByUserId(userId);
  ...
  async verifyRegistration(
    ...
    try {
      ...
      const existingCredential =
        await this.credentialRepository.findWebAuthnCredentialByCredentialId(id);
  ...
  async deleteCredential(userId: string, credentialId: string): Promise<boolean> {
    try {
      ...
      // Get credential
      const credential =
        await this.credentialRepository.findWebAuthnCredentialByCredentialId(credentialId);
      if (!credential) {
