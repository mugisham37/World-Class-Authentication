...
export class MagicLinkService {
  ...
  async sendMagicLink(
    ...
    try {
      ...
      if (
        passwordlessConfig.magicLink.allowedDomains &&
        passwordlessConfig.magicLink.allowedDomains.length > 0
      ) {
      ...
      if (activeLinks.length >= passwordlessConfig.magicLink.maxTokensPerUser) {
        throw new BadRequestError(
          `Too many active magic links (max: ${passwordlessConfig.magicLink.maxTokensPerUser})`
        );
      }
      ...
      if (passwordlessConfig.magicLink.reuseWindow > 0 && activeLinks.length > 0) {
        const recentLinks = activeLinks.filter(
          (link: MagicLink) =>
            Date.now() - link.createdAt.getTime() < passwordlessConfig.magicLink.reuseWindow * 1000
        );
