...
export class MfaService {
  ...
  async startFactorEnrollment(
    ...
    if (activeFactors.length >= mfaConfig.general.maxActiveMethods) {
      throw new BadRequestError(
        `Maximum number of active MFA methods (${mfaConfig.general.maxActiveMethods}) reached`
      );
    }
    ...
    try {
      switch (factorType) {
        ...
        case MfaFactorType.WEBAUTHN:
          enrollmentResult = await this.webAuthnService.startEnrollment(
            userId,
            factorName,
            factorData
          );
        ...
        case MfaFactorType.SMS:
          ...
          enrollmentResult = await this.smsMfaService.startEnrollment(
            userId,
            factorName,
            factorData['phoneNumber']
          );
          break;
  ...
  async completeFactorEnrollment(
    ...
    try {
      ...
      switch (factor.type) {
        case MfaFactorType.TOTP:
          verificationResult = await this.totpService.verifyEnrollment(
            factorId,
        ...
        case MfaFactorType.WEBAUTHN:
          ...
          break;
        case MfaFactorType.SMS:
          verificationResult = await this.smsMfaService.verifyEnrollment(
            factorId,
            verificationData['code']
          );
          break;
        case MfaFactorType.EMAIL:
          verificationResult = await this.emailMfaService.verifyEnrollment(
            factorId,
            verificationData['code']
          );
          break;
        case MfaFactorType.RECOVERY_CODE:
          ...
          break;
        case MfaFactorType.PUSH_NOTIFICATION:
          verificationResult = await this.pushNotificationService.verifyEnrollment(
            factorId,
    ...
    try {
      ...
      switch (factor.type) {
        case MfaFactorType.TOTP:
          ...
          break;
        case MfaFactorType.WEBAUTHN:
          verificationResult = await this.webAuthnService.verifyChallenge(
            challengeId,
            response,
            metadata
          );
        ...
        case MfaFactorType.RECOVERY_CODE:
          ...
          break;
        case MfaFactorType.PUSH_NOTIFICATION:
          verificationResult = await this.pushNotificationService.verifyChallenge(
            challengeId,
            response,
            metadata
          );
   ...
   */
  async selectFactorsForChallenge(
    userId: string,
  ...
  async regenerateRecoveryCodes(userId: string): Promise<string[]> {
    ...
    const existingFactors = await this.mfaFactorRepository.findByUserIdAndType(
      userId,
