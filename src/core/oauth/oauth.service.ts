...
export class OAuthService {
   ...
   */
  private async verifyPkceChallenge(
    verifier: string,
    challenge: string,
    method: string
  ): Promise<boolean> {
  ...
  async registerClient(
    ...
    try {
      ...
      const clientType =
        registrationRequest['token_endpoint_auth_method'] === 'none' ? 'public' : 'confidential';
      ...
      await this.clientRepository.create({
        ...
        allowedScopes: registrationRequest['scope']
          ? registrationRequest['scope'].split(' ')
          : ['openid'],
  ...
  private validateClientRegistrationRequest(request: Record<string, any>): void {
      ...
      try {
        ...
        if (!url.protocol.startsWith('http')) {
          throw new BadRequestError(
            `Invalid redirect URI: ${uri}. Must use HTTP or HTTPS protocol.`
          );
        }
      } catch (error) {
    ...
    if (
      ...
      ![
        ...
        'client_secret_jwt',
        'private_key_jwt',
      ].includes(request['token_endpoint_auth_method'])
    ) {
