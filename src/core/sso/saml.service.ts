...
import { SSOEvent, SAMLRequest, SAMLResponse, SAMLUser, IdPConfiguration, AttributeMapping, SSOSession, SAMLLogoutRequest, SAMLLogoutResponse, IdPMetadata } from "./sso-events"
...
export class SAMLService {
  ...
  private initializeIdentityProviders(): void {
    ...
    for (const idpConfig of samlConfig.identityProviders) {
      try {
        ...
        logger.error(`Error initializing identity provider: ${idpConfig.name} (${idpConfig.id})`, { error })
      }
    }
  }
  ...
  async registerIdP(idpConfig: IdPConfiguration): Promise<any> {
    try {
      ...
      if (existingIdP) {
        throw new BadRequestError(`Identity provider with entity ID '${idpConfig.entityId}' already exists`)
      }
   ...
   */
  async generateLogoutRequest(sessionId: string, relayState?: string): Promise<SAMLLogoutRequest | null> {
    try {
      // Find session
   ...
   */
  private mapAttributes(attributes: Record<string, string[]>, mapping: Record<string, string>): Record<string, any> {
    ...
    } else if (!result.email && attributes["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress"] && attributes["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress"].length > 0) {
      result.email = attributes["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress"][0]
    }
  ...
  private getIdPMetadata(idpConfig: any): IdPMetadata {
    return {
      ...
      nameIdFormats: [idpConfig.nameIdFormat || "urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress"],
  ...
  private loadCertificate(idpConfig: any): string {
    ...
    if (idpConfig.certificatePath) {
      try {
        ...
        logger.error(`Error loading certificate from path: ${idpConfig.certificatePath}`, { error })
