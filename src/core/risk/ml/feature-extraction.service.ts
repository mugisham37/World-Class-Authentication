...
export class FeatureExtractionService {
   ...
   */
  async extractAuthenticationFeatures(
    userId: string,
    ...
    try {
      ...
      // Use search method which supports limit and sorting
      const auditLogResult = await this.auditLogRepository.search({
        userId: userId,
        limit: 50,
      });
      ...
      const behaviorFeatures = this.extractBehaviorFeatures(auditLogs);
      ...
      // Return basic features on error with default values for all required properties
      return {
        // Temporal features
   ...
   */
  private extractTemporalFeatures(loginHistory: any[]): TemporalFeatures {
    // Initialize features
    ...
    try {
      ...
      for (let i = 1; i < loginHistory.length; i++) {
        if (
          ...
          const interval =
            (loginHistory[i - 1].timestamp.getTime() - loginHistory[i].timestamp.getTime()) /
            (60 * 60 * 1000); // hours
      ...
      const weekendLogins = loginHistory.filter(
        login =>
          login &&
          login.timestamp &&
          (login.timestamp.getDay() === 0 || login.timestamp.getDay() === 6)
      ).length;
      features['weekend_login_ratio'] =
        loginHistory.length > 0 ? weekendLogins / loginHistory.length : 0;
      ...
      features['business_hours_login_ratio'] =
        loginHistory.length > 0 ? businessHoursLogins / loginHistory.length : 0;
  ...
  private extractLocationFeatures(
    ...
    try {
      ...
      if (context['countryCode'] && loginHistory.length > 0) {
        features['is_new_country'] = !loginHistory.some(
          login => login && login.countryCode === context['countryCode']
        );
      }

      if (context['regionCode'] && loginHistory.length > 0) {
        features['is_new_region'] = !loginHistory.some(
          login => login && login.regionCode === context['regionCode']
        );
      }

      if (context['city'] && loginHistory.length > 0) {
        features['is_new_city'] = !loginHistory.some(
          login => login && login.city === context['city']
        );
      }
   ...
   */
  private extractDeviceFeatures(loginHistory: any[], context: Record<string, any>): DeviceFeatures {
    // Initialize features
    ...
    try {
      ...
      if (deviceHistory.length > 0) {
        const firstSeen = deviceHistory.reduce((oldest, login) => {
          return login && login.timestamp && (!oldest || login.timestamp < oldest)
            ? login.timestamp
            : oldest;
        }, null);
      ...
      loginHistory.forEach(login => {
        if (login && login.deviceFingerprint) {
          deviceFrequency[login.deviceFingerprint] =
            (deviceFrequency[login.deviceFingerprint] || 0) + 1;
        }
      });
      ...
      const recentLogins = loginHistory.filter(
        login =>
          login && login.timestamp && login.timestamp >= new Date(Date.now() - 24 * 60 * 60 * 1000)
      );
      const recentDevices = new Set(
        recentLogins
          .filter(login => login && login.deviceFingerprint)
   ...
   */
  private extractBehaviorFeatures(auditLogs: any[]): BehaviorFeatures {
    // Initialize features
    ...
    try {
      ...
      features['failed_login_ratio'] =
        loginAttempts.length > 0 ? failedLogins.length / loginAttempts.length : 0;
      ...
      const mfaChallenges = auditLogs.filter(
        log => log && log.action === 'MFA_CHALLENGE_GENERATED'
      );
      ...
      if (lastActivity && lastActivity.timestamp) {
        features['inactive_period_before_login'] =
          (Date.now() - lastActivity.timestamp.getTime()) / (60 * 60 * 1000); // hours
      }
   ...
   */
  private extractSessionFeatures(
    activeSessions: any[],
    context: Record<string, any>
  ): SessionFeatures {
    // Initialize features
    ...
    try {
      ...
      activeSessions.forEach(session => {
        if (session && session.deviceFingerprint) {
          deviceFrequency[session.deviceFingerprint] =
            (deviceFrequency[session.deviceFingerprint] || 0) + 1;
        }
      });
      ...
      features['has_overlapping_session'] = activeSessions.some(session => {
        ...
        const sessionEnd = session.expiresAt
   ...
   */
  private extractUserFeatures(user: any): UserFeatures {
    // Initialize features
    ...
    try {
      ...
      if (user.passwordUpdatedAt) {
        features['password_age'] =
          (Date.now() - user.passwordUpdatedAt.getTime()) / (24 * 60 * 60 * 1000);
      }
