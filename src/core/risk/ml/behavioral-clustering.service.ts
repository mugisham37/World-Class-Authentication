...
export class BehavioralClusteringService {
  ...
  async getUserCluster(userId: string, context: Record<string, any>): Promise<Record<string, any>> {
    try {
      ...
      if (cachedCluster) {
        logger.debug("Using cached behavioral cluster", { userId, clusterId: cachedCluster["clusterId"] })
   ...
   */
  private async determineCluster(features: Record<string, any>, loginHistory: any[]): Promise<Record<string, any>> {
    try {
   ...
   */
  private evaluateClusterSimilarity(cluster: Record<string, any>, context: Record<string, any>): Record<string, any> {
    try {
