...
export class AnomalyDetectionService {
   ...
   */
  async detectAnomalies(userId: string, context: Record<string, any>): Promise<Record<string, any>> {
    try {
      ...
      anomalyScores["temporal_anomaly"] = await this.detectTemporalAnomalies(features, loginHistory)

      // Detect location anomalies
      anomalyScores["location_anomaly"] = await this.detectLocationAnomalies(features, loginHistory)

      // Detect device anomalies
      anomalyScores["device_anomaly"] = await this.detectDeviceAnomalies(features, loginHistory)

      // Detect behavior anomalies
      anomalyScores["behavior_anomaly"] = await this.detectBehaviorAnomalies(features, riskAssessments)
      ...
      logger.debug("Anomaly detection completed", { userId, overallScore: anomalyScores["overall_anomaly"] })
   ...
   */
  private async detectTemporalAnomalies(features: Record<string, any>, loginHistory: any[]): Promise<number> {
    try {
      ...
      if (features["login_time_variance"] < 2) {
        ...
        const loginHours = loginHistory.filter((login) => login.timestamp).map((login) => login.timestamp.getHours())
      ...
      if (features["login_day_variance"] < 1) {
        ...
        const loginDays = loginHistory.filter((login) => login.timestamp).map((login) => login.timestamp.getDay())
      ...
      if (
        ...
        const zScore = Math.abs(
          (features["inactive_period_before_login"] - features["login_interval_mean"]) / features["login_interval_std"],
        )
   ...
   */
  private async detectLocationAnomalies(features: Record<string, any>, loginHistory: any[]): Promise<number> {
    try {
      ...
      if (features["location_diversity"] === 1 && (features["is_new_country"] || features["is_new_region"])) {
   ...
   */
  private async detectDeviceAnomalies(features: Record<string, any>, loginHistory: any[]): Promise<number> {
    try {
      if (loginHistory.length < 3) {
        return 0 // Not enough data for reliable detection
      }
   ...
   */
  private async detectBehaviorAnomalies(features: Record<string, any>, riskAssessments: any[]): Promise<number> {
    try {
      ...
      if (features["session_duration_mean"] > 0 && features["session_duration_std"] > 0 && features["session_age_mean"] > 0) {
        // Calculate z-score for current session age
        const zScore = Math.abs(
          (features["session_age_mean"] - features["session_duration_mean"]) / features["session_duration_std"],
        )
    ...
    try {
      ...
      if (anomalyScores["temporal_anomaly"] && anomalyScores["temporal_anomaly"] > 30) {
        details["temporal"] = {
          ...
          unusual_time: features["login_time_variance"] !== undefined && features["login_time_variance"] < 2,
          unusual_day: features["login_day_variance"] !== undefined && features["login_day_variance"] < 1,
          high_frequency: features["login_frequency_daily"] !== undefined && 
          ...
          unusual_interval:
            ...
            features["inactive_period_before_login"] > features["login_interval_mean"] + 2 * features["login_interval_std"],
        }
      }
      ...
      if (anomalyScores["location_anomaly"] && anomalyScores["location_anomaly"] > 30) {
        details["location"] = {
          ...
          impossible_travel: features["max_travel_speed"] !== undefined && features["max_travel_speed"] > 1000,
          unusual_distance: features["distance_from_last_login"] !== undefined && features["distance_from_last_login"] > 500,
        }
      }
      ...
      if (anomalyScores["behavior_anomaly"] && anomalyScores["behavior_anomaly"] > 30) {
        details["behavior"] = {
          ...
          high_failed_logins: features["failed_login_ratio"] !== undefined && features["failed_login_ratio"] > 0.3,
          recent_password_reset: features["password_reset_frequency"] !== undefined && 
                                features["password_age"] !== undefined && 
                                features["password_reset_frequency"] > 2 && features["password_age"] < 3,
          high_mfa_challenges: features["mfa_challenge_frequency"] !== undefined && features["mfa_challenge_frequency"] > 5,
          suspicious_actions: features["suspicious_action_frequency"] !== undefined && features["suspicious_action_frequency"] > 0,
          unusual_session:
            features["session_age_mean"] !== undefined && 
            features["session_duration_mean"] !== undefined && 
            features["session_duration_std"] !== undefined && 
            features["session_age_mean"] > features["session_duration_mean"] + 2 * features["session_duration_std"],
          overlapping_sessions: Boolean(features["has_overlapping_session"]),
          long_inactivity: features["inactive_period_before_login"] !== undefined && features["inactive_period_before_login"] > 14 * 24,
        }
      }
