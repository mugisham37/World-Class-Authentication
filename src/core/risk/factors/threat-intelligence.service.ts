...
export class ThreatIntelligenceService {
  ...
  async assessRisk(userId: string | null, context: Record<string, any>): Promise<number> {
    try {
      ...
      const isThreatActor = await this.checkKnownThreatActor(context.ipAddress, context.deviceFingerprint)
      if (isThreatActor) {
      ...
      const isInBotnet = await this.checkBotnetActivity(context.ipAddress, context.deviceFingerprint)
      if (isInBotnet) {
   ...
   */
  private async checkKnownThreatActor(ipAddress?: string, deviceFingerprint?: string): Promise<boolean> {
    try {
   ...
   */
  private async checkBotnetActivity(ipAddress?: string, deviceFingerprint?: string): Promise<boolean> {
    try {
      if (!ipAddress && !deviceFingerprint) {
        return false
      }
  ...
  private checkPhishingIndicators(context: Record<string, any>): Record<string, boolean> | false {
    try {
      ...
      if (context.userAgent) {
        const suspiciousUserAgents = ["PhishingBrowser", "DataCollector", "WebScraper", "HeadlessChrome", "PhantomJS"]
  ...
  private async checkAttackPatterns(context: Record<string, any>): Promise<string | null> {
    try {
      ...
      if (context.failedAttempts && context.failedAttempts > 10 && context.timeWindow && context.timeWindow < 300) {
        return "brute_force"
      }
      ...
      if (context.sessionId && context.originalIp && context.ipAddress && context.originalIp !== context.ipAddress) {
