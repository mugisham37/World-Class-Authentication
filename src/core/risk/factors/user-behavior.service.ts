...
export class UserBehaviorService {
  ...
  async assessRisk(userId: string, context: Record<string, any>): Promise<number> {
    try {
      ...
      if (loginLocationScore > 0) {
        riskScore = Math.max(riskScore, riskFactors.unusualLoginLocation * (loginLocationScore / 100))
      }
      ...
      if (loginFrequencyScore > 0) {
        riskScore = Math.max(riskScore, riskFactors.unusualLoginFrequency * (loginFrequencyScore / 100))
      }
      ...
      if (activityPatternScore > 0) {
        riskScore = Math.max(riskScore, riskFactors.unusualActivityPattern * (activityPatternScore / 100))
      }
      ...
      if (accountSwitchingScore > 0) {
        riskScore = Math.max(riskScore, riskFactors.rapidAccountSwitching * (accountSwitchingScore / 100))
      }
  ...
  private async generateUserBehaviorProfile(userId: string): Promise<UserBehaviorProfile | null> {
    try {
      ...
      for (const login of loginHistory) {
        ...
        if (login.regionCode) {
          profile.loginLocations.regions[login.regionCode] = (profile.loginLocations.regions[login.regionCode] || 0) + 1
        }

        if (login.city) {
          profile.loginLocations.cities[login.city] = (profile.loginLocations.cities[login.city] || 0) + 1
        }
        ...
        // Devices
        if (login.browser) {
          profile.devices.browsers[login.browser] = (profile.devices.browsers[login.browser] || 0) + 1
        }

        if (login.os) {
          profile.devices.operatingSystems[login.os] = (profile.devices.operatingSystems[login.os] || 0) + 1
        }

        if (login.deviceType) {
          profile.devices.deviceTypes[login.deviceType] = (profile.devices.deviceTypes[login.deviceType] || 0) + 1
        }
      }
        ...
        for (const session of sessions) {
          if (session.length > 0) {
            ...
            if (firstActivity.timestamp && lastActivity.timestamp) {
              const duration = (lastActivity.timestamp.getTime() - firstActivity.timestamp.getTime()) / 1000 / 60 // minutes
  ...
  private groupActivitiesBySessions(activities: any[]): any[][] {
    ...
    for (const activity of sortedActivities) {
      if (currentSession.length === 0) {
        ...
        const timeDiff = (activity.timestamp.getTime() - lastActivity.timestamp.getTime()) / 1000 / 60 // minutes
    ...
    try {
      ...
      for (let i = 1; i < recentLogins.length; i++) {
        if (recentLogins[i - 1].timestamp && recentLogins[i].timestamp) {
          const timeBetween =
            (recentLogins[i - 1].timestamp.getTime() - recentLogins[i].timestamp.getTime()) / 1000 / 60 / 60 // hours
  ...
  private async assessActivityPattern(
    ...
    try {
      ...
      if (deviceType && profile.devices.deviceTypes) {
        const totalDevices = Object.values(profile.devices.deviceTypes).reduce((sum, count) => sum + Number(count), 0)
        if (totalDevices === 0) {
          return 0
        }
   ...
   * @returns Risk score (0-100)
   */
  private async assessAccountSwitching(userId: string, context: Record<string, any>): Promise<number> {
    try {
      if (!userId || !context) {
        return 0
      }
      ...
      if (ipAddress) {
        ...
        if (recentLoginsFromIp && recentLoginsFromIp.length > 0) {
          // Count unique users
          const uniqueUsers = new Set(recentLoginsFromIp.map((login) => login.userId).filter(Boolean))
      ...
      if (deviceFingerprint) {
        ...
        const recentLoginsFromDevice = await this.userLoginHistoryRepository.findRecentByDeviceFingerprint(
          deviceFingerprint,
        ...
        if (recentLoginsFromDevice && recentLoginsFromDevice.length > 0) {
          // Count unique users
          const uniqueUsers = new Set(recentLoginsFromDevice.map((login) => login.userId).filter(Boolean))
  ...
  async updateProfileWithLogin(userId: string, loginData: Record<string, any>): Promise<void> {
    try {
      ...
      if (loginData.city) {
        profile.loginLocations.cities[loginData.city] = (profile.loginLocations.cities[loginData.city] || 0) + 1
      }
      ...
      if (loginData.browser) {
        profile.devices.browsers[loginData.browser] = (profile.devices.browsers[loginData.browser] || 0) + 1
      }

      if (loginData.os) {
        profile.devices.operatingSystems[loginData.os] = (profile.devices.operatingSystems[loginData.os] || 0) + 1
      }

      if (loginData.deviceType) {
        profile.devices.deviceTypes[loginData.deviceType] = (profile.devices.deviceTypes[loginData.deviceType] || 0) + 1
      }
