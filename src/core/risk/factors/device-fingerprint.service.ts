...
export class DeviceFingerprintService {
    ...
    try {
      ...
      if (userAgent.includes("iPhone")) {
        ...
        if (modelMatch) {
          ...
          const brandMatch = model.match(/^(Samsung|LG|Huawei|Xiaomi|OnePlus|Google|Motorola|Sony|Nokia|HTC)/i)
          if (bruandMatch) {
            brand = brandMatch[1]
          }
        }
  ...
  private detectAnomalies(
    ...
    try {
      ...
      if ((device?.type === "mobile" || device?.type === "tablet") && data['touchSupport'] === false) {
  ...
  private detectSuspiciousDevice(deviceData: DeviceFingerprintData): number {
    ...
    try {
      ...
      if (features && !features['cookies'] && !features['localStorage'] && !features['sessionStorage']) {
        suspiciousScore += 40
      }
  ...
  private async isMultiAccountDevice(deviceHash: string, currentUserId: string): Promise<boolean> {
    try {
      ...
      logger.error("Error checking if device is used by multiple accounts", { error, deviceHash, currentUserId })
      return false
    }
  }
  ...
  async registerDevice(userId: string, deviceData: RawDeviceFingerprint): Promise<string> {
    try {
      ...
      const existingDevice = await this.userDeviceRepository.findByUserIdAndHash(userId, parsedData.hash)
  ...
  async trustDevice(userId: string, deviceId: string): Promise<boolean> {
    try {
      ...
      await this.userDeviceRepository.update(deviceId, {
        isTrusted: true,
        trustExpiresAt: new Date(Date.now() + riskConfig.deviceFingerprint.trustDeviceDuration * 1000),
      })
